# Treated Municipal Wastewater Dataset
## Dataset Source
The initial dataset – consists of 17 XLSX files representing treated municipal wastewater historical data from 2008 to 2024 - was obtained 
from [https://data.ontario.ca/dataset/municipal-treated-wastewater-effluent](https://data.ontario.ca/dataset/municipal-treated-wastewater-effluent). This is a public data source provided by Ministry: Environment, Conservation and Parks (Ontario, Canada).
Some of the original files (in CSV format) are too large for GitHub. Therefore, you can download them from [Google Drive](https://drive.google.com/drive/u/0/folders/1kZ8xXnKVMvC3uFeiLHynqrmYuU6sivR5)

---

## Interactive Tableau Public Story

The dataset was used to create an interactive Tableau Public Story. View the Tableau Story [here](https://public.tableau.com/views/MunicipalWWTPsAcrossMinistryRegionsOntarioCanada2008-2024/MunicipalTreatedWW_Ontario_Canada_2008-2024?:language=en-US&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link) 

---

## Notebook (in R Language)

A notebook (in R Language) was created here, in Kaggle. View the Kaggle Notebook [here](https://www.kaggle.com/code/salehireza2083/treated-municipal-wastewater-data-patterns)

---

## SQL Queries in Google BigQuery

View the GitHub SQL Queries [here](https://github.com/reza-salehi125/treated_municipal_wastewater_ontario_canada_cleaning/blob/main/treated_municipal_wastewater.sql)
